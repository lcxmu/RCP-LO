...
def rigid_transform_3d(A, B, weights=None):
    """
    Compute the rigid transformation (rotation and translation) that aligns
    two sets of 3D points A and B.
    
    Args:
        A: Source points, shape (N, 3)
        B: Target points, shape (N, 3)
        weights: Optional weights for each point, shape (N,)
    ...
    # Compute the weighted centroids of A and B
    ...
    # Compute the weighted covariance matrix H
    H = np.dot((A_centered * weights[:, np.newaxis]).T, B_centered)
